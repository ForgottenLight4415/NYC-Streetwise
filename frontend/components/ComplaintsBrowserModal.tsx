...
export function ComplaintsBrowserModal({
  ...
  const requestKey = [lat, lng, tier, radiusMeters, months, bucket, status, offset, pageSize].join("|");
  ...
  }, [requestKey, lat, lng, tier, radiusMeters, months, bucket, status, offset, pageSize]);
  ...
  const bucketOptions = [
    { value: "all", label: "All" },
    ...TIER_BUCKETS[tier].map((b) => ({ value: b, label: CATEGORY_LABEL[b] ?? b })),
  ];
  const statusOptions = [
    { value: "all", label: "All" },
    ...COMPLAINT_STATUS_OPTIONS.map((o) => ({ value: o.value as string, label: o.label })),
  ];
  ...
  return (
    ...
    >
      <div
        ...
        className="flex max-h-[85dvh] w-full max-w-2xl flex-col rounded-[var(--radius-lg)] outline-none"
        style={{
      ...
      >
        ...
        >
          <div className="min-w-0">
            <h2 className="truncate text-lg font-semibold text-[color:var(--text-primary)]">
        ...
        {drill ? (
          <DrillInPane
            ...
            onOffsetChange={(next) => setDrill({ ...drill, offset: next, items: null })}
          ...
          <>
            ...
            >
              <FilterChips
                label="Time window"
                options={TREND_WINDOW_OPTIONS.map((m) => ({ value: m, label: String(m) }))}
                value={months}
                onChange={changeFilter((m: number) => setMonths(m as TrendWindow))}
                suffix="mo"
              />
              <FilterChips
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 sm:px-6">
              {error ? (
                <FallbackList error={error} items={fallback} onSelect={setSelected} />
              ) : groups === null ? (
                <FactRotator />
              ) : groups.length === 0 ? (
                ...
                <ul className="flex flex-col divide-y divide-[color:var(--gridline)]">
                  {groups.map((g) => (
                    <li key={`${g.day}|${g.type}`}>
                      <button
                        type="button"
                        onClick={() => setDrill({ group: g, items: null, total: g.total, offset: 0 })}
                      ...
                      >
                        <div className="min-w-0">
                          <p className="truncate text-[color:var(--text-primary)]">{g.type}</p>
                          <p className="font-data text-xs text-[color:var(--text-muted)]">
                            {formatDay(g.day)}
                          </p>
                        </div>
                        <span className="flex shrink-0 items-center gap-2.5">
                          <StatusDots counts={g.counts} />
                          <span className="font-data text-xs font-medium text-[color:var(--text-primary)]">
                            {g.total}
                          </span>
            ...
            <div className="shrink-0 px-5 pb-5 sm:px-6 sm:pb-6">
              {truncated && (
                <p className="pb-2 text-[11px] text-[color:var(--text-muted)]">
                  This address has more history than we can hold — showing the most recent records only.
                </p>
              )}
              {groups !== null && !error && (
                <Pager
...
function StatusDots({ counts }: { counts: Record<ComplaintStatus, number> }) {
  const present = (Object.keys(counts) as ComplaintStatus[]).filter((s) => counts[s] > 0);
  ...
  return (
    <span className="flex items-center gap-1.5">
      {present.map((s) => (
        ...
        >
          <span className="h-1.5 w-1.5 rounded-full" style={{ background: `var(${STATUS_VAR[s]})` }} />
          {mixed && counts[s]}
...
function FallbackList({
  ...
  return (
    <div className="py-4">
      ...
      >
        {error} Showing the most recent complaints instead — filters and full history are
        unavailable.
      </p>
...
function DrillInPane({
  ...
  return (
    <>
      <div className="shrink-0 border-b px-5 py-2.5 sm:px-6" style={{ borderColor: "var(--border-hairline)" }}>
        <button
          type="button"
          onClick={onBack}
          className="inline-flex min-h-8 items-center gap-1.5 text-xs font-semibold text-[color:var(--brand-ink)]"
        >
            ...
            {drill.items.map((c) => (
              <li key={c.id}>
                <button
                  ...
                  className="flex min-h-11 w-full items-center justify-between gap-3 rounded-md py-2.5 text-left text-sm transition-colors hover:bg-[color:var(--surface-2)]"
                >
                  <p className="min-w-0 truncate text-[color:var(--text-primary)]">{c.label}</p>
